    ...
    def __init__(self, host_list, notable, notitle, filter):
        if host_list is not None:
            if filter is None:
                filter = ''
            if not notable:
                template = "{0:3}|{1:15}|{2:15}|{3:6}|{4:6}|{5:20}|{6:20}"
            if not notitle:
                if notable:
                    print("ID\tIP\tHost\t"
                            "Proto\tPort\t"
                    ...
                    print template.format("ID", "IP", "Host",
                                                        "Proto", "Port",
                                                        "Product", "Version")
                        ...
                        if filter in grep_text:
                            if (notable):
                                ...
                                print template.format(service.service_id,
                                    nic.nic_ip, service.service_proto, service.service_port,
                                    service.service_product, service.service_version)
            ...
            for network in network_list:
                ...
                if filter in greptext:
                    if (notable):
                        ...
                        print template.format(network.network_id, network.network_ip,
                            network.network_prefix, network.network_desc)
        ...
        if host_list is not None:
            ...
            if not notitle:
                if notable:
                    ...
                    print template.format("ID", "Name", "STS", "PWN", "ROT", "OS")
            for host in host_list:
